...
public class TextureAtlas {
    ...
    public void load(){
        ...
        if(data != null) {
            ...
            glUniform1i(glGetUniformLocation(RectRenderer2D.getInstance().getShaderID(), "uTexture[" + (id) + "]"), id);

            int err = glGetError();
            if (err != 0) {
                Logger.getInstance().error("ERROR CREATING TEXTURE ATLAS: " + err);
            }
        }
        else{
            Logger.getInstance().error("COULD NOT LOAD TEXTURE ATLAS DATA");
        }
        if(preGenerated){
